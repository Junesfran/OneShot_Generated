...
def delete_user_by_id(id: int, user: User):
    ...
    deleted: bool = user_controller.delete_user(user.idUsuario)
...
def logout(user: User):
    ...
    if(user_controller.logout(user.idUsuario) == True):
        return {"success": "logued out"}, 200
    else:
        return {"error": "couln not log out"}, 500